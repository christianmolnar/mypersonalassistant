...
// import prompts from '../../data/imagePrompts'; // No longer needed
...
export default function ImageGeneratorPage() {
  ...
  useEffect(() => {
    fetch('/api/prompts')
      .then(res => res.json())
      .then(data => setPrompts(data.map((p: any) => p.text)))
      .catch(() => setPrompts([]));
  }, []);
  ...
  const handlePromptChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
    ...
    setCustomPrompt(e.target.value); // Show selected prompt in textarea for editing
  };
  ...
  const handleSavePrompt = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setPrompts([newPrompt.text, ...prompts]);
        setSelectedPrompt(newPrompt.text);
      } else {
